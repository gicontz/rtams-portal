...
$student_id_trim = preg_replace('/\s+/','',$student_id);
...
if($student_id != $student_id_trim)
{
echo "<script language='javascript'>alert('Don't use spaces in Username')</script>"; 
}
$check_query = "select * from tbl_student where email = '$email' or student_id = '$student_id' ";
...
$insert_query = "INSERT INTO `tbl_student` (`date`, `student_id`, `password`, `email`, `firstname`, `middlename`, `lastname`, `image`, `contact`, `acct_status`) VALUES ('$date', '$student_id', '$password', '$email', '$firstname', '$middlename', '$lastname', '$image', '$contact', 'activate')";
   
    
if(mysqli_query($con,$insert_query))
{  
echo "<script language='javascript'>alert('Student Account has been Added')</script>";    
header("refresh:1; add-student.php");   
...
<input type="number" name="student_id" class="form-control" placeholder="Student ID" value="<?php if(isset($student_id)){echo $student_id;}?>" required> 
</div> <!--ADD USERNAME-->
...
<div class="col-md-12">       
<center><input type="submit" value="Create Student Account" name="add_student" class="btn btn-default btn-md"></center>
<hr>
</div> 
</form> 
<!--ADD STUDENT ACCOUNT-->
